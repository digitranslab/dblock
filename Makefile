...
env ?= .env
open_browser ?= true
path = src/backend/base/flowai/frontend
workers ?= 1
async ?= true
lf ?= false
ff ?= true
...
patch: ## bump the version in flowai and flowai-base
	@echo 'Patching the version'
	@poetry version patch
	@echo 'Patching the version in flowai-base'
...
build_frontend: ## build the frontend static files
	...
	$(call CLEAR_DIRS,src/backend/base/flowai/frontend)
	@echo 'Copying build files...'
	@cp -r src/frontend/build/. src/backend/base/flowai/frontend
...
clean_npm_cache:
	...
	$(call CLEAR_DIRS,src/frontend/node_modules src/frontend/build src/backend/base/flowai/frontend)
...
lint: install_backend ## run linters
	@uv run mypy --namespace-packages -p "flowai"
...
setup_devcontainer: ## set up the development container
	...
	make install_frontend
	make build_frontend
	uv run flowai --frontend-path src/frontend/build
...
ifdef login
	...
	FLOWAI_AUTO_LOGIN=$(login) uv run uvicorn \
		--factory flowai.main:create_app \
		--host 0.0.0.0 \
		--port $(port) \
...
ifdef base
	...
	make build_frontend
	make build_flowai_base args="$(args)"
endif
...
dockerfile_build_be: dockerfile_build
	...
	@docker build --rm \
		--build-arg FLOWAI_IMAGE=flowai:${VERSION} \
		-f ${DOCKERFILE_BACKEND} \
		-t flowai_backend:${VERSION} .
