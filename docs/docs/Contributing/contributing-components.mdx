...
New components are added as objects of the [`Component`](https://github.com/minerva-ai/minerva/blob/main/src/backend/base/minerva/custom/custom_component/component.py) class.

Dependencies are added to the [pyproject.toml](https://github.com/minerva-ai/minerva/blob/main/pyproject.toml#L148) file.
...
2. Write your processor as an object of the [`Component`](https://github.com/minerva-ai/minerva/blob/main/src/backend/base/minerva/custom/custom_component/component.py) class. You'll create a new class, `DataFrameProcessor`, that will inherit from `Component` and override the base class's methods.
...
class DataFrameProcessor(Component):
    ...
    documentation: str = "https://docs.minerva.org/components-dataframe-processor"
...
5. Save the `dataframe_processor.py` to the `src > backend > base > minerva > components` directory.
This example adds a data component, so add it to the `/data` directory.

6. Add the component dependency to `src > backend > base > minerva > components > data > __init__.py` as `from .DataFrameProcessor import DataFrameProcessor`.
You can view the [/data/__init__.py](https://github.com/minerva-ai/minerva/blob/dev/src/backend/base/minerva/components/data/__init__.py) in the Langflow repository.

7. Add any new dependencies to the [pyproject.toml](https://github.com/minerva-ai/minerva/blob/main/pyproject.toml#L20) file.

8. Submit documentation for your component. For this example component, you would submit documentation to the [Data components page](https://github.com/minerva-ai/minerva/blob/main/docs/docs/Components/components-data.mdx).
