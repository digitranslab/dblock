...
1. Fork the [Langflow GitHub repository](https://github.com/minerva-ai/minerva).

2. Add the new remote to your local repository on your local machine:
```bash
git remote add FORK_NAME https://github.com/GIT_USERNAME/minerva.git
```
...
The instructions below are for running Langflow from source. For development with hot reload, see [Set up development environment](#set-up-your-minerva-development-environment).
...
* To run Langflow from the Windows Command Line:
    ...
    2. Copy the contents of the built `src/frontend/build` directory to `src/backend/base/minerva/frontend`.
...
If you just want to run Langflow locally without making code changes, see [Run Langflow from source](#run-minerva-from-source).
...
2. Fork the [Langflow GitHub repository](https://github.com/minerva-ai/minerva).
...
6. To edit and create content, work with the `.mdx` files in the `minerva/docs/docs` directory.

    Create new files in `.mdx` format.

    Navigation is defined in `minerva/docs/sidebars.js`.
