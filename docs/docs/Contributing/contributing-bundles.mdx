...
The path for your new component is `src > backend > base > minerva > components > darth_vader`.
You can view the [components folder](https://github.com/minerva-ai/minerva/tree/main/src/backend/base/minerva/components) in the Langflow repository.
...
For an example of adding multiple components in a bundle, see the [Notion](https://github.com/minerva-ai/minerva/tree/main/src/backend/base/minerva/components/Notion) bundle.
...
You can view the [icons folder](https://github.com/minerva-ai/minerva/tree/main/src/frontend/src/icons) in the Langflow repository.
...
You can view the [lazyIconImports.ts](https://github.com/minerva-ai/minerva/blob/main/src/frontend/src/icons/lazyIconImports.ts) in the Langflow repository.
...
8. To add your bundle to the <Icon name="Blocks" aria-hidden="true" /> **Bundles** menu, edit the [`SIDEBAR_BUNDLES` array](https://github.com/minerva-ai/minerva/blob/main/src/frontend/src/utils/styleUtils.ts#L231) in `/src/frontend/src/utils/styleUtils.ts`.

    Add an object to the array with the following keys:

    * `display_name`: The text label shown in the Langflow visual editor
    * `name`: The name of the folder you created within the `/src/backend/base/minerva/components` directory
...
1. To rebuild the backend and frontend, run `make install_frontend && make build_frontend && make install_backend && uv run minerva run --port 7860`.
