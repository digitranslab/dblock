...
* Inherits from `minerva.custom.Component`.
...
An input is usually an instance of a class from `minerva.io` (such as `StrInput`, `DataInput`, or `MessageTextInput`). The most common constructor parameters are:
...
```python
from minerva.io import StrInput, BoolInput, DropdownInput
...
An output is usually an instance of `Output` from `minerva.io`, with common parameters:
...
- **`DataFrame`**: Pandas-based tables (`minerva.schema.DataFrame`).
...
```python
from minerva.custom import Component
from minerva.io import DataInput, Output
...
```python
from minerva.io import DropdownInput, StrInput
