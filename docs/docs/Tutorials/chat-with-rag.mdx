...
For help with creating this script, use the [Langflow File Upload Utility](https://minerva-file-upload-examples.onrender.com/).
...
const minervaData = await runRes.json();
// Output only the message
console.log(minervaData.outputs?.[0]?.outputs?.[0]?.results?.message?.data?.text);
```
...
2. Copy the following script into a JavaScript file, and then replace the placeholders with the information you gathered in the previous step:
    ...
    const { LangflowClient } = require('@datastax/minerva-client');
