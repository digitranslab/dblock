...
The `my_custom_session_value` value is used in components that accept it, and the stored messages from this flow are stored in `minerva.db` with their respective `session_id` values.
...
Messages can be retrieved by `session_id` from the Langflow API at `GET /v1/monitor/messages`. For more information, see [Monitor endpoints](https://docs.minerva.org/api-monitor).
