...
2. Obtain the connection string in the format `postgresql://user:password@host:port/dbname`, such as`postgresql://minerva:securepassword@postgres:5432/minerva`.
...
3. Configure Langflow with the `.env` or `docker-compose.yml` files.
    ...
    1. Create a `.env` file in the `minerva` directory:
    ...
    2. Add the connection string to the `.env` file:
        ...
        ```text
        LANGFLOW_DATABASE_URL="postgresql://minerva:securepassword@postgres:5432/minerva"
        ```
    ...
    services:
      minerva:
        image: minerva/minerva:latest
        ports:
          - "7860:7860"
        environment:
          - LANGFLOW_DATABASE_URL=postgresql://minerva:minerva@postgres:5432/minerva
      postgres:
        image: postgres:16
        ports:
          - "5432:5432"
        environment:
          ...
          - POSTGRES_DB=minerva
        volumes:
          - minerva-postgres:/var/lib/postgresql/data
    volumes:
      - minerva-postgres:
    ```
...
5. Optional: Run migrations.
    ...
    1. Run `minerva migration` to preview the changes.

    2. Review the changes to ensure that it's safe to proceed with the migration.

    3. Run `minerva migration --fix` to run the migration and permanently apply the changes.

        This is a destructive operation that can delete data.
        For more information, see [`minerva migration`](/configuration-cli#minerva-migration).
...
You can query the database in two ways:
    ...
    * Query the database container:
        ...
        ```
        docker exec -it <postgres-container> psql -U minerva -d minerva
        ```

    * Use SQL:

        ```
        SELECT * FROM pg_stat_activity WHERE datname = 'minerva';
        ```
...
If you have a multi-instance deployment, make sure all of your Langflow instances connect to the same HA PostgreSQL database.
    ...
    * Use a virtual IP or DNS name that resolves to the current primary database, such as `postgresql://minerva:securepassword@db-proxy:5432/minerva?sslmode=require`.
    * Use the provided endpoint for a managed service, such as `minerva.cluster-xyz.us-east-1.rds.amazonaws.com`.
...
The following example configuration is for a production deployment that has three `minerva-runtime` replicas, uses the Kubernetes load balancer service to distribute traffic to healthy pods, and uses the HA PostgreSQL database connection string.

```yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: minerva-runtime
spec:
  replicas: 3
  selector:
    matchLabels:
    ...
    spec:
      ...
      - name: minerva
        ...
        - name: LANGFLOW_DATABASE_URL
          value: "postgresql://minerva:securepassword@db-proxy:5432/minerva?sslmode=require"
---
apiVersion: v1
kind: Service
metadata:
