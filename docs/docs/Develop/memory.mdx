...
- **Langflow Desktop**:
    - **macOS**: `/Users/<username>/.minerva/data/database.db`
    - **Windows**: `C:\Users\<name>\AppData\Roaming\com.LangflowDesktop\data\database.db`
- **Langflow OSS**
    - **macOS/Windows/Linux/WSL with `uv pip install`**: `<path_to_venv>/lib/python3.12/site-packages/minerva/minerva.db` (Python version can vary. Database isn't shared between virtual environments because it is tied to the venv path.)
    - **macOS/Windows/Linux/WSL with `git clone`**: `<path_to_clone>/src/backend/base/minerva/minerva.db`
...
The following tables are stored in `minerva.db`:
...
For more information, see the database models in the [source code](https://github.com/minerva-ai/minerva/tree/main/src/backend/base/minerva/services/database/models).
