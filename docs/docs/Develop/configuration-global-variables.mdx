...
Langflow automatically generates global variables based on [`constants.py`](https://github.com/minerva-ai/minerva/blob/main/src/lfx/src/lfx/services/settings/constants.py) if it detects any matching environment variables.
...
4. Start Langflow with the `.env` file:
   ...
   ```bash
   VARIABLE1="VALUE1" VARIABLE2="VALUE2" uv run minerva run --env-file .env
   ```
