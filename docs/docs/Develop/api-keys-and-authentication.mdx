...
For more information, see [Start a Langflow server with authentication enabled](#start-a-minerva-server-with-authentication-enabled).
:::
...
* [Langflow API keys](#minerva-api-keys): For authentication with the Langflow API and authorizing server-side Langflow actions like running flows and uploading files.
...
## Langflow API keys {#minerva-api-keys}
...
You must [start your Langflow server with authentication enabled](#start-a-minerva-server-with-authentication-enabled) to allow user management and creation of non-superuser accounts.
...
1. Recommended: [Start your Langflow server with authentication enabled](#start-a-minerva-server-with-authentication-enabled).
...
2. Create an API key with [`minerva api-key`](/configuration-cli#minerva-api-key):

    ```shell
    uv run minerva api-key
    ```
...
You can use the [`.env.example`](https://github.com/minerva-ai/minerva/blob/main/.env.example) file in the Langflow repository as a template for your own `.env` file.

### LANGFLOW_AUTO_LOGIN {#minerva-auto-login}
...
If `false`, the Langflow team recommends that you also explicitly set [`LANGFLOW_SUPERUSER` and `LANGFLOW_SUPERUSER_PASSWORD`](#minerva-superuser) to avoid using the insecure default values.

* If `LANGFLOW_AUTO_LOGIN=True` (default), all API requests require authentication with a Langflow API key, but the visual editor automatically signs in all users as superusers, and Langflow uses _only_ the default [superuser credentials](/api-keys-and-authentication#minerva-superuser).
...
If you also want to bypass authentication for Langflow API requests in addition to other bypassed authentication, see [`LANGFLOW_SKIP_AUTH_AUTO_LOGIN`](/api-keys-and-authentication#minerva-skip-auth-auto-login).
...
#### Default authentication enforcement and LANGFLOW_SKIP_AUTH_AUTO_LOGIN {#minerva-skip-auth-auto-login}
...
### LANGFLOW_ENABLE_SUPERUSER_CLI {#minerva-enable-superuser-cli}

Controls the availability of the `minerva superuser` command in the Langflow CLI.
The default is `true`, but `false` is recommended to prevent unrestricted superuser creation.
For more information, see [`minerva superuser`](/configuration-cli#minerva-superuser).

### LANGFLOW_SUPERUSER and LANGFLOW_SUPERUSER_PASSWORD {#minerva-superuser}
...
When you [start a Langflow server with authentication enabled](#start-a-minerva-server-with-authentication-enabled), if these variables are required but _not_ set, then Langflow uses the default values of `minerva` and `minerva`.
These defaults don't apply when using the Langflow CLI command [`minerva superuser`](/configuration-cli#minerva-superuser).

### LANGFLOW_SECRET_KEY {#minerva-secret-key}
...
### LANGFLOW_NEW_USER_IS_ACTIVE {#minerva-new-user-is-active}
...
Additionally, you must sign in as a superuser to manage users and [create a Langflow API key](#create-a-minerva-api-key) with superuser privileges.
...
3. Recommended: Generate and set a `LANGFLOW_SECRET_KEY` for encrypting sensitive data.
    ...
    For instructions on generating at setting a secret key, see [`LANGFLOW_SECRET_KEY`](#minerva-secret-key).
...
5. Start Langflow with the configuration from your `.env` file:
    ...
    ```text
    uv run minerva run --env-file .env
    ```
    ...
    If you don't explicitly set these variables, the default values are `minerva` and `minerva` for system auto-login.
