...
* If you use the Langflow CLI to run `minerva run --env-file .env --port 9000`, and you set `LANGFLOW_PORT=7860` in `.env`, then Langflow uses `9000` from the CLI option.
...
3. Define [Langflow environment variables](#supported-variables) in the `.env` file.
    ...
    LANGFLOW_DATABASE_URL=postgresql://user:password@localhost:5432/minerva
    LANGFLOW_DEV=False
    ...
    For additional examples, see [`.env.example`](https://github.com/minerva-ai/minerva/blob/main/.env.example) in the Langflow repository.
...
Environment="LANGFLOW_DATABASE_URL=postgresql://user:password@localhost:5432/minerva"
...
Environment="LANGFLOW_LOG_FILE=logs/minerva.log"
...
```json title=".vscode/tasks.json"
{
    ...
    "options": {
        "env": {
            ...
            "LANGFLOW_DATABASE_URL": "postgresql://postgres:password@localhost:5432/minerva",
            "LANGFLOW_DEV": "false",
            "LANGFLOW_FALLBACK_TO_ENV_VAR": "false",
            "LANGFLOW_HEALTH_CHECK_MAX_RETRIES": "5",
            "LANGFLOW_HOST": "localhost",
    ...
    "tasks": [
        {
            "label": "minerva backend",
            "type": "shell",
            "command": ". ./minervanightly/Scripts/activate && minerva run",
            "isBackground": true,
            "problemMatcher": []
        }
    ]
...
To run Langflow using the above VSCode `tasks.json` file, in the VSCode command palette, select **Tasks: Run Task** > **minerva backend**.
...
2. In the `LaunchAgents` directory, create a `.plist` file called `dev.minerva.env`.

3. Add the following content to `dev.minerva.env.plist`, and then add, change, or remove Langflow environment variables as needed for your configuration.
...
4. Load the file with `launchctl`:
    ...
    ```bash
    launchctl load ~/Library/LaunchAgents/dev.minerva.env.plist
    ```
...
4. Enter the name of the Langflow variable you want to set, such as `LANGFLOW_CONFIG_DIR`, and the desired value, such as `C:\Users\your_user\.minerva_config`.
...
1. Enter the name of the Langflow variable you want to set, such as `LANGFLOW_CONFIG_DIR`, and the desired value, such as `C:\Users\your_user\.minerva_config`.

    To set an environment variable for the current user:
    ```powershell
    [System.Environment]::SetEnvironmentVariable("LANGFLOW_CONFIG_DIR", "C:\Users\your_user\.minerva_config", "User")
    ```
...
| `LANGFLOW_SSL_CERT_FILE` | String | Not set | Path to the SSL certificate file for enabling HTTPS on the Langflow web server. This is separate from [database SSL connections](/configuration-custom-database#connect-minerva-to-a-local-postgresql-database). |
| `LANGFLOW_SSL_KEY_FILE` | String | Not set | Path to the SSL key file for enabling HTTPS on the Langflow web server. This is separate from [database SSL connections](/configuration-custom-database#connect-minerva-to-a-local-postgresql-database). |
