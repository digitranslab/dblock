...
This guide walks you through setting up an external database for Langflow by replacing the default SQLite connection string `sqlite:///./minerva.db` with PostgreSQL, both in local and containerized environments.
...
For an example, see the [`docker-compose.yml`](https://github.com/minerva-ai/minerva/blob/main/docker_example/docker-compose.yml) file in the Langflow repository.
...
To start the Langflow and PostgreSQL services with the example Docker Compose file, navigate to the `minerva/docker_example` directory, and then run `docker-compose up`.
...
Your container name may vary.
    ...
    ```bash
    docker exec -it docker-test-postgres-1 psql -U minerva -d minerva
    ```

5. Query the database for active connections:

    ```sql
    minerva=# SELECT * FROM pg_stat_activity WHERE datname = 'minerva';
    ```
