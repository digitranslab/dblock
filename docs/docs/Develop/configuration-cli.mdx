...
If Langflow is installed globally or added to your PATH, you can execute the CLI directly with `minerva`.
...
```bash
uv run minerva run --remove-api-keys
```
...
```bash
uv run minerva run --no-remove-api-keys
```
...
### minerva api-key {#minerva-api-key}

Creates a Langflow API key.

You must be a superuser to create API keys with the CLI.
For more information, see [Langflow API keys](/api-keys-and-authentication#minerva-api-keys).
...
You can find the copy target directory by running `which minerva`.

The following files are copied if they exist in the cache directory:

* `minerva.db`: The main Langflow database, stored in the user cache directory
* `minerva-pre.db`: The pre-release database, if it exists

<Tabs groupId="Invocation">
  <TabItem value="uv (recommended)" label="uv (recommended)" default>

```bash
uv run minerva copy-db
```
...
* **Fix mode**: Applies the migrations to update the database schema.
    ...
    :::warning
    `minerva migration --fix` is a destructive operation that can delete data.
    Always run `minerva migration` first to preview the changes.
    :::
...
3. Run fix mode to apply the changes:
  ...
  ```bash
  minerva migration --fix
  ```

  </TabItem>
</Tabs>

### minerva run {#minerva-run}
...
| `--backend-only` | `--no-backend-only` (false) | Boolean | Whether to run Langflow's backend service only (no frontend). Omit or use `--no-backend-only` to start both the frontend and backend. See [Start Langflow in headless mode](#start-minerva-in-headless-mode). |
...
| `--env-file` | Not set | String | The path to the `.env` file containing Langflow environment variables. See [Start Langflow with a specific .env file](#start-minerva-with-a-specific-env-file). |
...
| `--log-file` | `logs/minerva.log` | String | The path to the log file for Langflow. |
...
#### Start Langflow with a specific .env file {#start-minerva-with-a-specific-env-file}
...
```bash
uv run minerva run --env-file PATH/TO/LANGFLOW/.env
```
...
```bash
minerva run --env-file PATH/TO/LANGFLOW/.env
```

  </TabItem>
</Tabs>

#### Start Langflow in headless mode {#start-minerva-in-headless-mode}
...
```bash
uv run minerva run --backend-only
```
...
```bash
minerva run --backend-only
```

  </TabItem>
</Tabs>

### minerva superuser {#minerva-superuser}
...
```bash
uv run minerva superuser --username [NAME] --password [PASSWORD] [OPTIONS]
```
...
```bash
minerva superuser --username [NAME] --password [PASSWORD] [OPTIONS]
```
...
For more information, see [`LANGFLOW_SUPERUSER` and `LANGFLOW_SUPERUSER_PASSWORD`](/api-keys-and-authentication#minerva-superuser).

#### Disable CLI superuser creation

The `minerva superuser` command is controlled by the [`LANGFLOW_ENABLE_SUPERUSER_CLI`](/api-keys-and-authentication#minerva-enable-superuser-cli) environment variable:

* **`LANGFLOW_ENABLE_SUPERUSER_CLI=True` (default)**: The `minerva superuser` command is available, and superuser creation is unrestricted.
* **`LANGFLOW_ENABLE_SUPERUSER_CLI=False` (recommended)**: Disables the `minerva superuser` command.
...
To disable the `minerva superuser` command, you must set `LANGFLOW_ENABLE_SUPERUSER_CLI=False` in your Langflow `.env` file, and then [start Langflow with your `.env` file](#start-minerva-with-a-specific-env-file).