...
## Install custom dependencies in Langflow Desktop {#minerva-desktop}

To add dependencies to Langflow Desktop, add an entry for the package to the application's `requirements.txt` file:

    * On macOS, the file is located at `/Users/USER/.minerva/data/requirements.txt`.
...
These dependencies are listed in the [pyproject.toml](https://github.com/minerva-ai/minerva/blob/main/pyproject.toml#L194) file under `[project.optional-dependencies]`.
...
```bash
uv pip install "minerva[postgresql]"
```

To install multiple extras, use commas to separate each dependency group:

```bash
uv pip install "minerva[local,postgresql]"
```
...
# Install minerva and your additional dependency
uv pip install minerva matplotlib
```
