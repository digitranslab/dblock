...
The [Langflow integrated development environment (IDE) Helm chart](https://github.com/minerva-ai/minerva-helm-charts/tree/main/charts/minerva-ide) is designed to provide a complete environment for developers to create, test, and debug their flows. It includes both the Langflow API and visual editor.
...
2. Install Langflow with the default options in the `minerva` namespace:

	```shell
	helm install minerva-ide minerva/minerva-ide -n minerva --create-namespace
	```
...
1. Make the Langflow API accessible from your local machine at port 7860:
    ...
    ```shell
    kubectl port-forward -n minerva svc/minerva-service-backend 7860:7860
    ```

2. Make the visual editor accessible from your local machine at port 8080:

    ```shell
    kubectl port-forward -n minerva svc/minerva-service 8080:8080
    ```
...
You can modify the Langflow IDE Helm chart's [`values.yaml`](https://github.com/minerva-ai/minerva-helm-charts/blob/main/charts/minerva-ide/values.yaml) file to customize your deployment.
...
To specify a different Langflow version, set the `minerva.backend.image.tag` and `minerva.frontend.image.tag` values to your preferred version.
For example:
...
If you don't want to use the built-in PostgreSQL chart, set `postgresql.enabled` to `false`, and then configure the database connection in `minerva.backend.externalDatabase`:
...
To configure scaling for the Langflow IDE Helm chart deployment, you must set `replicaCount` (horizontal scaling) and `resources` (vertical scaling) for both the `minerva.backend` and `minerva.frontend`.
