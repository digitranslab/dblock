...
* `/minerva-config-dir`: This folder is referenced in the Dockerfile as the location for your Langflow deployment's configuration files, database, and logs.
...
The base Langflow Docker image includes the Langflow core dependencies because it uses `minerva/minerva:latest` as the parent image.
...
RUN mkdir /app/flows
RUN mkdir /app/minerva-config-dir
WORKDIR /app

# Copy flows, minerva-config-dir, and docker.env to the container
COPY flows /app/flows
COPY minerva-config-dir /app/minerva-config-dir
...
ENV LANGFLOW_CONFIG_DIR=/app/minerva-config-dir
ENV LANGFLOW_COMPONENTS_PATH=/app/components
ENV LANGFLOW_LOG_ENV=container

# Command to run the Langflow server on port 7860
EXPOSE 7860
CMD ["minerva", "run", "--backend-only", "--env-file","/app/.env","--host", "0.0.0.0", "--port", "7860"]
```
...
1. Build the Docker image:
    ...
    ```bash
    docker build -t minerva-pokedex:1.2.0 .
    ```

2. Run the Docker container to start your Langflow server:

    ```bash
    docker run -p 7860:7860 minerva-pokedex:1.2.0
    ```
