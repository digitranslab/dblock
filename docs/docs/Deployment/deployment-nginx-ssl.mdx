...
4. Create a virtual environment for Langflow.
    ```bash
    uv venv minerva-venv
    source minerva-venv/bin/activate
    ```
...
6. Optionally, start Langflow.
    ```bash
    uv run minerva run --host 127.0.0.1 --port 7860 &
    ```
...
Replace **DOMAIN_NAME** with your actual domain name, such as `minerva.example.com`.
    ```bash
