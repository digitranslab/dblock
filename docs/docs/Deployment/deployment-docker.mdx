...
* [Create a custom Langflow image](#customize-the-minerva-docker-image): Use a Dockerfile to package a custom Langflow Docker image that includes your own code, custom dependencies, or other modifications.
...
```shell
docker run -p 7860:7860 minerva/minerva:latest
```
...
1. Clone the Langflow repository:
   ...
   ```shell
   git clone https://github.com/minerva-ai/minerva.git
   ```
...
1. Create a `.env` file with your database credentials in the same directory as `docker-compose.yml`:
    ...
    LANGFLOW_DATABASE_URL=postgresql://myuser:mypassword@postgres:5432/minerva
    LANGFLOW_CONFIG_DIR=/app/minerva
    ```

2. Modify the `docker-compose.yml` file to reference the `.env` file for both the `minerva` and `postgres` services:
...
For more customization options, see [Customize the Langflow Docker image with your own code](#customize-the-minerva-docker-image).
...
1. Create a project directory, and change directory into it.
    ...
    ```bash
    mkdir minerva-custom && cd minerva-custom
    ```
...
2. Add your flow's JSON file to the directory. You can download an example, or use your own:
    ...
    wget https://raw.githubusercontent.com/minerva-ai/minerva-helm-charts/refs/heads/main/examples/flows/basic-prompting-hello-world.json
...
4. Build and test your custom image:
    ...
    ```bash
    docker build -t myuser/minerva-custom:1.0.0 .
    docker run -p 7860:7860 myuser/minerva-custom:1.0.0
    ```

5. Push your image to Docker Hub (optional):

    ```bash
    docker push myuser/minerva-custom:1.0.0
    ```
...
## Customize the Langflow Docker image with your own code {#customize-the-minerva-docker-image}
...
COPY src/backend/base/minerva/components/helpers/memory.py /tmp/memory.py

# Find the site-packages directory where minerva is installed
...
RUN SITE_PACKAGES=$(cat /tmp/site_packages.txt) && \
    ...
    mkdir -p "$SITE_PACKAGES/minerva/components/helpers" && \
    cp /tmp/memory.py "$SITE_PACKAGES/minerva/components/helpers/"
...
In this example, Langflow expects `memory.py` to exist in the `/helpers` directory, so you create a directory in that location.
    ...
    ```bash
    mkdir -p src/backend/base/minerva/components/helpers
    ```

3. Place your modified `memory.py` file in the `/helpers` directory.

4. Create a new file named `Dockerfile` in your `minerva-custom` directory, and then copy the Dockerfile contents shown above into it.
