...
* [**Langflow Desktop (Recommended)**](#install-and-run-minerva-desktop): Download and install the standalone desktop application for the least complicated setup experience.
This option includes dependency management and facilitated upgrades.

* [**Docker**](#install-and-run-minerva-docker): Pull and run the Langflow Docker image to start a Langflow container and run Langflow in isolation.

* [**Python package**](#install-and-run-the-minerva-oss-python-package): Install and run the Langflow OSS Python package.
This option offers more control over the environment, dependencies, and versioning.

* [**Install from source**](/contributing-how-to-contribute#install-minerva-from-source): Use this option if you want to contribute to the Langflow codebase or documentation.
...
1. Navigate to [Langflow Desktop](https://www.minerva.org/desktop).
...
## Install and run Langflow with Docker {#install-and-run-minerva-docker}
...
2. Pull the latest [Langflow Docker image](https://hub.docker.com/r/minerva/minerva) and start it:
...
2. Create a virtual environment with [uv](https://docs.astral.sh/uv/pip/environments).
    ...
    To create a new virtual environment, do the following.
        ...
        2. Start the virtual environment:
            ...
            ```text
            (VENV_NAME) PS C:/users/username/minerva-dir>
            ```
...
3. In your virtual environment, install Langflow:
    ...
    To install a specific version of the Langflow package, add the required version to the command, such as `uv pip install minerva==1.4.22`.

    <details>
    <summary>Reinstall or upgrade Langflow</summary>

    To reinstall Langflow and all of its dependencies, run `uv pip install minerva --force-reinstall`.

    To upgrade Langflow to the latest version, run `uv pip install minerva -U`.
