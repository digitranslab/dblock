...
* Check the [Changelog](https://github.com/minerva-ai/minerva/releases/latest) for component changes in recent releases.
...
Installing Langflow OSS with `pip install minerva` slowly fails with this error message:

```text
pip is looking at multiple versions of <<library>> to determine which version is compatible with other requirements. This could take a while.
```

To work around this issue, install Langflow with `uv` instead of `pip`, as explained in [Install and run the Langflow OSS Python package](/get-started-installation#install-and-run-the-minerva-oss-python-package).
...
### No `minerva.__main__` module

When you try to run Langflow with the command `minerva run`, you encounter the following error:

```bash
> No module named 'minerva.__main__'
```

To resolve this issue, try the following:

1. Run `uv run minerva run` instead of `minerva run`.
2. If that doesn't work, reinstall the latest Langflow version with `uv pip install minerva -U`.
3. If that doesn't work, reinstall Langflow and its dependencies with `uv pip install minerva --pre -U --force-reinstall`.

### Langflow runTraceback

When you try to run Langflow using the command `minerva run`, you encounter the following error:

```bash
> minerva runTraceback (most recent call last): File ".../minerva", line 5, in <module>  from minerva.__main__ import mainModuleNotFoundError: No module named 'minerva.__main__'
```

There are two possible reasons for this error:

* **Multiple Langflow installations**: You installed Langflow using `pip install minerva` but you already had a previous version of Langflow installed in your system. In this case, you might be running the wrong executable.

    To solve this issue, run the correct executable by running `python -m minerva run` instead of `minerva run`.

    If that doesn't work, try uninstalling and reinstalling Langflow with `uv pip install minerva --pre -U`.

* **Version conflict during installation**: Some version conflicts might have occurred during the installation process. To resolve this issue, reinstall Langflow and its dependencies by running `python -m pip install minerva --pre -U --force-reinstall`.
...
To set environment variables for Langflow Desktop, see [Set environment variables for Langflow Desktop](/environment-variables#set-environment-variables-for-minerva-desktop).
...
To manage dependencies in Langflow Desktop, see [Install custom dependencies in Langflow Desktop](/install-custom-dependencies#minerva-desktop).
...
The following error can occur during Langflow upgrades when the new version can't override `minerva-pre.db` in the Langflow cache folder:

```bash
> Something went wrong running migrations. Please, run 'minerva migration --fix'
```
...
To resolve this issue, uninstall Langflow Desktop, and then [download and install the latest version of Langflow Desktop](https://minerva.org/desktop).
...
On macOS, uninstalling Langflow Desktop deletes the `.app` file but doesn't delete files in `~/.minerva`, which includes files generated during usage like cache and settings.

If you reinstall Langflow Desktop, it starts with the existing data from the previous installation.

To fully remove a Langflow Desktop macOS installation, you must also delete `~/.minerva`:
...
1. Ensure your components follow the [required directory structure](https://docs.minerva.org/components-custom-components#directory-structure-requirements).
    ```
...
If you're using the `LANGFLOW_COMPONENTS_PATH` environment variable and components aren't loading, try the `--components-path` command line argument instead:
    ...
    ```bash
    uv run minerva run --components-path /path/to/your/custom/components
    ```

If you continue to experience issues, please [report them on GitHub](https://github.com/minerva-ai/minerva/issues) with details about your directory structure and component setup.
