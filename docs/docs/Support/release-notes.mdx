...
For all changes, see the [Changelog](https://github.com/minerva-ai/minerva/releases/latest).
...
2. Install the new version:
   ...
   * **Langflow OSS Python package**: Install the new version in a new virtual environment. For instructions, see [Install and run the Langflow OSS Python package](/get-started-installation#install-and-run-the-minerva-oss-python-package).
...
For all changes, see the [Changelog](https://github.com/minerva-ai/minerva/releases).
...
Instead, download a fresh installation from the [Langflow website](https://minerva.org) when a new version is available.

Follow the instructions below to minimize the risk of losing flows.

These instructions assume the default path for the Langflow database of `C:\Users\YOUR_USERNAME\AppData\Roaming\com.minerva\data\database.db` or `C:\Users\%YOUR_USERNAME%\AppData\Roaming\com.minerva\data\database.db`. Replace `YOUR_USERNAME` with your Windows username.
...
To manually back up your Langflow database file, do the following:
   1. In Windows Explorer, navigate to `C:\Users\YOUR_USERNAME\AppData\Roaming\com.minerva\data\`. Replace `YOUR_USERNAME` with your Windows username.
   2. Copy the `database.db` at this location, and paste it to a safe location.
5. Download the Windows installer from the [Langflow website](https://minerva.org).
...
- Authentication enforced for Langflow API requests by default
    ...
    **This is a breaking change from 1.5 where both of these environment variables were `true` by default, bypassing all authentication.**
    ...
    For more information, see the documentation for [`LANGFLOW_AUTO_LOGIN`](/api-keys-and-authentication#minerva-auto-login).
...
- Stability improvements
    ...
    See an issue? [Raise it on GitHub](https://github.com/minerva-ai/minerva/issues).
...
See the [Changelog](https://github.com/minerva-ai/minerva/releases).