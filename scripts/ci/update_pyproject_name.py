...
def update_uv_dep(pyproject_path: str, new_project_name: str) -> None:
    ...
    if new_project_name == "flowai-nightly":
        pattern = re.compile(r"flowai = \{ workspace = true \}")
        replacement = "flowai-nightly = { workspace = true }"
    elif new_project_name == "flowai-base-nightly":
        pattern = re.compile(r"flowai-base = \{ workspace = true \}")
        replacement = "flowai-base-nightly = { workspace = true }"
    else:
...
def main() -> None:
    if len(sys.argv) != ARGUMENT_NUMBER:
        msg = "Must specify project name and build type, e.g. flowai-nightly base"
        raise ValueError(msg)
