# Deploy Minerva on Google Cloud Platform

**Duration**: 45 minutes  

## Introduction

In this tutorial, you will learn how to deploy Minerva on [Google Cloud Platform](https://cloud.google.com/) (GCP) using Google Cloud Shell.
...
- A project on GCP where you want to deploy Minerva
...
In the next step, you'll configure the GCP environment and deploy Minerva.

## Configure the GCP environment and deploy Minerva
Run the deploy_minerva_gcp_spot.sh script to configure the GCP environment and deploy Minerva:

```sh  
gcloud config set project <walkthrough-project-id/>  
bash ./deploy_minerva_gcp_spot.sh
```

The script will:

1. Check if the required resources (VPC, subnet, firewall rules, and Cloud Router) exist and create them if needed
2. Create a startup script to install Python, Minerva, and Nginx
3. Create a Compute Engine VM instance with the specified configuration and startup script
4. Run Minerva to serve content on TCP port 7860
...
In the next step, you'll learn how to connect to the Minerva VM.

## Connect to the Minerva VM
To connect to your new Minerva VM, follow these steps:
...
<br>**or**
3. Run the following command to display the URL for your Minerva environment:
```bash
export MINERVA_IP=$(gcloud compute instances list --filter="NAME=minerva-dev" --format="value(EXTERNAL_IP)")

echo http://$MINERVA_IP:7860
```

4. Click on the Minerva URL in cloudshell to be greeted by the Minerva Dev environment

Congratulations! You have successfully deployed Minerva on Google Cloud Platform.
...
```sql
gcloud compute instances delete minerva-dev --zone us-central1-a --quiet
```
