...
from flowai.inputs.inputs import MessageTextInput
from flowai.memory import aget_messages, astore_message
...
class MessageStoreComponent(Component):
    ...
    description = "Stores a chat message or text into Flowai tables or an external memory."
    ...
    inputs = [
        ...
        HandleInput(
            ...
            input_types=["Memory"],
            info="The external memory to store the message. If empty, it will use the Flowai tables.",
        ),
        MessageTextInput(
            name="sender",
            display_name="Sender",
