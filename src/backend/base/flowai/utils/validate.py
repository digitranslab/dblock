...
from flowai.field_typing.constants import CUSTOM_COMPONENT_SUPPORTED_TYPES, DEFAULT_IMPORT_STRING
...
def create_class(code, class_name):
    ...
    # Replace from flowai import CustomComponent with from flowai.custom import CustomComponent
    code = code.replace("from flowai import CustomComponent", "from flowai.custom import CustomComponent")
    code = code.replace(
...
def get_default_imports(code_string):
    ...
    flowai_imports = list(CUSTOM_COMPONENT_SUPPORTED_TYPES.keys())
    necessary_imports = find_names_in_code(code_string, flowai_imports)
    flowai_module = importlib.import_module("flowai.field_typing")
    default_imports.update({name: getattr(flowai_module, name) for name in necessary_imports})
