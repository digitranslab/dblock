...
from flowai.api import health_check_router, log_router, router, router_v2
...
from flowai.interface.utils import setup_llm_caching
from flowai.logging.logger import configure
from flowai.middleware import ContentSizeLimitMiddleware
from flowai.services.deps import get_settings_service, get_telemetry_service
from flowai.services.utils import initialize_services, teardown_services
...
def get_lifespan(*, fix_migration=False, version=None):
    ...
    async def lifespan(_app: FastAPI):
        ...
        # Startup message
        if version:
            rprint(f"[bold green]Starting Flowai v{version}...[/bold green]")
        else:
            rprint("[bold green]Starting Flowai...[/bold green]")
        ...
        try:
            ...
            if "flowai migration --fix" not in str(exc):
            ...
            # Final message
            rprint("[bold red]Flowai shutdown complete[/bold red]")
...
def create_app():
    ...
    app = FastAPI(lifespan=lifespan, title="Flowai", version=__version__)
    app.add_middleware(
    ...
    if prome_port_str := os.environ.get("FLOWAI_PROMETHEUS_PORT"):
    ...
    if settings.mcp_server_enabled:
        from flowai.api.v1 import mcp_router
...
def get_static_files_dir():
    """Get the static files directory relative to Flowai's main.py file."""
...
if __name__ == "__main__":
    ...
    from flowai.__main__ import get_number_of_workers

    configure()
    uvicorn.run(
        "flowai.main:create_app",
        host="127.0.0.1",
        port=7860,
