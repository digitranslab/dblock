...
from flowai.services.task.backends.anyio import AnyIOBackend
from flowai.services.task.utils import get_celery_worker_status

if TYPE_CHECKING:
    from flowai.services.settings.service import SettingsService
    from flowai.services.task.backends.base import TaskBackend


def check_celery_availability():
    try:
        from flowai.worker import celery_app
...
class TaskService(Service):
    ...
    def get_backend(self) -> TaskBackend:
        if self.use_celery:
            from flowai.services.task.backends.celery import CeleryBackend
