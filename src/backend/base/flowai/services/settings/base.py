...
from flowai.services.settings.constants import VARIABLES_TO_GET_FROM_ENVIRONMENT
from flowai.utils.util_strings import is_valid_database_url
...
class Settings(BaseSettings):
    # Define the default FLOWAI_DIR
    config_dir: str | None = None
    # Define if flowai db should be saved in config dir or
    # in the flowai directory
    save_db_in_config_dir: bool = False
    """Define if flowai database should be saved in FLOWAI_CONFIG_DIR or in the flowai directory
    (i.e. in the package directory)."""

    dev: bool = False
    """If True, Flowai will run in development mode."""
    database_url: str | None = None
    """Database URL for Flowai. If not provided, Flowai will use a SQLite database."""
    database_connection_retry: bool = False
    """If True, Flowai will retry to connect to the database if it fails."""
    pool_size: int = 10
    ...
    """If set to True, Flowai will expose Prometheus metrics."""
    prometheus_port: int = 9090
    """The port on which Flowai will expose Prometheus metrics. 9090 is the default port."""
    ...
    store_url: str | None = "https://api.flowai.store"
    download_webhook_url: str | None = "https://api.flowai.store/flows/trigger/ec611a61-8460-4438-b187-a4f65e5559d4"
    like_webhook_url: str | None = "https://api.flowai.store/flows/trigger/64275852-ec00-45c1-984e-3bff814732da"
    ...
    with the same name in case Flowai fails to retrieve the variable value."""
    ...
    user_agent: str = "flowai"
    """User agent for the API calls."""
    backend_only: bool = False
    """If set to True, Flowai will not serve the frontend."""

    # Telemetry
    do_not_track: bool = False
    """If set to True, Flowai will not track telemetry."""
    telemetry_base_url: str = "https://flowai.gateway.scarf.sh"
    transactions_storage_enabled: bool = True
    """If set to True, Flowai will track transactions between flows."""
    vertex_builds_storage_enabled: bool = True
    """If set to True, Flowai will keep track of each vertex builds (outputs) in the UI for any flow."""

    # Config
    host: str = "127.0.0.1"
    """The host on which Flowai will run."""
    port: int = 7860
    """The port on which Flowai will run."""
    workers: int = 1
    """The number of workers to run."""
    log_level: str = "critical"
    """The log level for Flowai."""
    log_file: str | None = "logs/flowai.log"
    """The path to log file for Flowai."""
    ...
    """If set to True, Flowai will open the browser on startup."""
    auto_saving: bool = True
    """If set to True, Flowai will auto save flows."""
    auto_saving_interval: int = 1000
    """The interval in ms at which Flowai will auto save flows."""
    ...
    """If set to False, Flowai will not enable the MCP server."""
    mcp_server_enable_progress_notifications: bool = False
    """If set to False, Flowai will not send progress notifications in the MCP server."""

    @field_validator("dev")
    @classmethod
    def set_dev(cls, value):
        from flowai.settings import set_dev
    ...
    def set_flowai_dir(cls, value):
        if not value:
            ...
            # Create a .flowai directory inside the cache directory
    ...
    def set_database_url(cls, value, info):
        ...
        logger.debug("No database_url provided, trying FLOWAI_DATABASE_URL env variable")
        if flowai_database_url := os.getenv("FLOWAI_DATABASE_URL"):
            value = flowai_database_url
            logger.debug("Using FLOWAI_DATABASE_URL env variable.")
        else:
            logger.debug("No database_url env variable, using sqlite database")
            # Originally, we used sqlite:///./flowai.db
            ...
            from flowai.utils.version import is_pre_release as flowai_is_pre_release

            version = get_version_info()["version"]
            is_pre_release = flowai_is_pre_release(version)
            ...
            if info.data["save_db_in_config_dir"]:
                ...
                logger.debug(f"Saving database to flowai directory: {database_dir}")

            pre_db_file_name = "flowai-pre.db"
            db_file_name = "flowai.db"
    ...
    def set_components_path(cls, value):
        if os.getenv("FLOWAI_COMPONENTS_PATH"):
            logger.debug("Adding FLOWAI_COMPONENTS_PATH to components_path")
            flowai_component_path = os.getenv("FLOWAI_COMPONENTS_PATH")
            if Path(flowai_component_path).exists() and flowai_component_path not in value:
                if isinstance(flowai_component_path, list):
                    for path in flowai_component_path:
                        if path not in value:
                            value.append(path)
                    logger.debug(f"Extending {flowai_component_path} to components_path")
                elif flowai_component_path not in value:
                    value.append(flowai_component_path)
                    logger.debug(f"Appending {flowai_component_path} to components_path")
    ...
    model_config = SettingsConfigDict(validate_assignment=True, extra="ignore", env_prefix="FLOWAI_")
