...
if TYPE_CHECKING:
    from flowai.services.store.schema import ListComponentResponse
...
# Get the latest released version of flowai (https://pypi.org/project/flowai/)
async def get_lf_version_from_pypi():
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get("https://pypi.org/pypi/flowai/json")
        if response.status_code != httpx.codes.OK:
            return None
        return response.json()["info"]["version"]
    except Exception:  # noqa: BLE001
        logger.opt(exception=True).debug("Error getting the latest version of flowai from PyPI")
        return None
