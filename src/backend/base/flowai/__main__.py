...
from flowai.initial_setup.setup import get_or_create_default_folder
from flowai.logging.logger import configure, logger
from flowai.main import setup_app
from flowai.services.database.utils import session_getter
from flowai.services.deps import get_db_service, get_settings_service, session_scope
from flowai.services.settings.constants import DEFAULT_SUPERUSER
from flowai.services.utils import initialize_services
from flowai.utils.version import fetch_latest_version, get_version_info
...
def run_on_mac_or_linux(host, port, log_level, options, app):
    webapp_process = Process(target=run_flowai, args=(host, port, log_level, options, app))
...
def run_on_windows(host, port, log_level, options, app) -> None:
    """Run the Flowai server on Windows."""
    print_banner(host, port)
    run_flowai(host, port, log_level, options, app)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "flowai")
        'A new version of flowai is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=flowai_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if flowai_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int) -> None:
    ...
    # Use flowai.utils.version to get the version info
    version_info = get_version_info()
    flowai_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= flowai_is_pre_release(flowai_version)  # Update pre-release status

    notice = build_version_notice(flowai_version, package_name)
    ...
    info_text = (
        ...
        "[bold][link=https://github.com/digitranslab/flowai]GitHub Repo[/link][/bold] :star2:"
    )
    telemetry_text = (
        "We collect anonymous usage data to improve Flowai.\n"
...
def run_flowai(host, port, log_level, options, app) -> None:
    """Run Flowai server on localhost."""
    if platform.system() == "Windows":
        import uvicorn
        ...
        from flowai.server import FlowaiApplication

        server = FlowaiApplication(app, options)
...
def superuser(
    ...
    log_level: str = typer.Option("error", help="Logging level.", envvar="FLOWAI_LOG_LEVEL"),
) -> None:
    """Create a superuser."""
    ...
    async def _create_superuser():
        ...
        async with session_getter(db_service) as session:
            from flowai.services.auth.utils import create_super_user
...
# command to copy the flowai database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'flowai.db' and 'flowai-pre.db' files from the cache directory to the current
    directory.
    ...
    cache_dir = Path(user_cache_dir("flowai"))
    db_path = cache_dir / "flowai.db"
    pre_db_path = cache_dir / "flowai-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from flowai.services.database.models.api_key import ApiKey, ApiKeyCreate
            from flowai.services.database.models.api_key.crud import create_api_key, delete_api_key
