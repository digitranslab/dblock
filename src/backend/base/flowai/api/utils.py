...
from flowai.services.auth.utils import get_current_active_user
from flowai.services.database.models import User
...
from flowai.services.database.models.vertex_builds.model import VertexBuildTable
from flowai.services.deps import get_session, session_scope
from flowai.services.store.utils import get_lf_version_from_pypi

if TYPE_CHECKING:
    from flowai.services.chat.service import ChatService
    from flowai.services.store.schema import StoreComponentCreate
...
async def check_flowai_version(component: StoreComponentCreate) -> None:
    ...
    flowai_version = await get_lf_version_from_pypi()
    if flowai_version is None:
        raise HTTPException(status_code=500, detail="Unable to verify the latest version of Flowai")
    if flowai_version != component.last_tested_version:
        logger.warning(
            f"Your version of Flowai ({component.last_tested_version}) is outdated. "
            f"Please update to the latest version ({flowai_version}) and try again."
        )
