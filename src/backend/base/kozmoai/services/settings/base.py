...
from kozmoai.services.settings.constants import VARIABLES_TO_GET_FROM_ENVIRONMENT
from kozmoai.utils.util_strings import is_valid_database_url
...
class Settings(BaseSettings):
    # Define the default KOZMOAI_DIR
    config_dir: str | None = None
    # Define if kozmoai db should be saved in config dir or
    # in the kozmoai directory
    save_db_in_config_dir: bool = False
    """Define if kozmoai database should be saved in KOZMOAI_CONFIG_DIR or in the kozmoai directory
    (i.e. in the package directory)."""

    dev: bool = False
    """If True, Kozmoai will run in development mode."""
    database_url: str | None = None
    """Database URL for Kozmoai. If not provided, Kozmoai will use a SQLite database."""
    database_connection_retry: bool = False
    """If True, Kozmoai will retry to connect to the database if it fails."""
    pool_size: int = 10
    ...
    """If set to True, Kozmoai will expose Prometheus metrics."""
    prometheus_port: int = 9090
    """The port on which Kozmoai will expose Prometheus metrics. 9090 is the default port."""
    ...
    store_url: str | None = "https://api.kozmoai.store"
    download_webhook_url: str | None = "https://api.kozmoai.store/flows/trigger/ec611a61-8460-4438-b187-a4f65e5559d4"
    like_webhook_url: str | None = "https://api.kozmoai.store/flows/trigger/64275852-ec00-45c1-984e-3bff814732da"
    ...
    with the same name in case Kozmoai fails to retrieve the variable value."""
    ...
    """If set to True, Kozmoai will keep track of each vertex builds (outputs) in the UI for any flow."""

    # Config
    host: str = "127.0.0.1"
    """The host on which Kozmoai will run."""
    port: int = 7860
    """The port on which Kozmoai will run."""
    workers: int = 1
    """The number of workers to run."""
    log_level: str = "critical"
    """The log level for Kozmoai."""
    log_file: str | None = "logs/kozmoai.log"
    """The path to log file for Kozmoai."""
    ...
    """If set to True, Kozmoai will open the browser on startup."""
    auto_saving: bool = True
    """If set to True, Kozmoai will auto save flows."""
    auto_saving_interval: int = 1000
    """The interval in ms at which Kozmoai will auto save flows."""
    ...
    """If set to False, Kozmoai will not enable the MCP server."""
    mcp_server_enable_progress_notifications: bool = False
    """If set to False, Kozmoai will not send progress notifications in the MCP server."""

    @field_validator("dev")
    @classmethod
    def set_dev(cls, value):
        from kozmoai.settings import set_dev
    ...
    def set_kozmoai_dir(cls, value):
        if not value:
            ...
            # Create a .kozmoai directory inside the cache directory
    ...
    def set_database_url(cls, value, info):
        ...
        logger.debug("No database_url provided, trying KOZMOAI_DATABASE_URL env variable")
        if kozmoai_database_url := os.getenv("KOZMOAI_DATABASE_URL"):
            value = kozmoai_database_url
            logger.debug("Using KOZMOAI_DATABASE_URL env variable.")
        else:
            logger.debug("No database_url env variable, using sqlite database")
            # Originally, we used sqlite:///./kozmoai.db
            ...
            from kozmoai.utils.version import is_pre_release as kozmoai_is_pre_release

            version = get_version_info()["version"]
            is_pre_release = kozmoai_is_pre_release(version)
            ...
            if info.data["save_db_in_config_dir"]:
                ...
                logger.debug(f"Saving database to kozmoai directory: {database_dir}")

            pre_db_file_name = "kozmoai-pre.db"
            db_file_name = "kozmoai.db"
    ...
    def set_components_path(cls, value):
        if os.getenv("KOZMOAI_COMPONENTS_PATH"):
            logger.debug("Adding KOZMOAI_COMPONENTS_PATH to components_path")
            kozmoai_component_path = os.getenv("KOZMOAI_COMPONENTS_PATH")
            if Path(kozmoai_component_path).exists() and kozmoai_component_path not in value:
                if isinstance(kozmoai_component_path, list):
                    for path in kozmoai_component_path:
                        if path not in value:
                            value.append(path)
                    logger.debug(f"Extending {kozmoai_component_path} to components_path")
                elif kozmoai_component_path not in value:
                    value.append(kozmoai_component_path)
                    logger.debug(f"Appending {kozmoai_component_path} to components_path")
    ...
    model_config = SettingsConfigDict(validate_assignment=True, extra="ignore", env_prefix="KOZMOAI_")
