...
from kozmoai.initial_setup.setup import get_or_create_default_folder
from kozmoai.logging.logger import configure, logger
from kozmoai.main import setup_app
from kozmoai.services.database.utils import session_getter
from kozmoai.services.deps import get_db_service, get_settings_service, session_scope
from kozmoai.services.settings.constants import DEFAULT_SUPERUSER
from kozmoai.services.utils import initialize_services
from kozmoai.utils.version import fetch_latest_version, get_version_info
...
def run_on_mac_or_linux(host, port, log_level, options, app):
    webapp_process = Process(target=run_kozmoai, args=(host, port, log_level, options, app))
...
def run_on_windows(host, port, log_level, options, app) -> None:
    """Run the Kozmoai server on Windows."""
    print_banner(host, port)
    run_kozmoai(host, port, log_level, options, app)
...
def build_version_notice(current_version: str, package_name: str) -> str:
    ...
    Example:
        >>> build_version_notice("1.0.0", "kozmoai")
        'A new version of kozmoai is available: 1.1.0'
    """
    with suppress(httpx.ConnectError):
        latest_version = fetch_latest_version(package_name, include_prerelease=kozmoai_is_pre_release(current_version))
        if latest_version and pkg_version.parse(current_version) < pkg_version.parse(latest_version):
            release_type = "pre-release" if kozmoai_is_pre_release(latest_version) else "version"
...
def print_banner(host: str, port: int) -> None:
    ...
    # Use kozmoai.utils.version to get the version info
    version_info = get_version_info()
    kozmoai_version = version_info["version"]
    package_name = version_info["package"]
    is_pre_release |= kozmoai_is_pre_release(kozmoai_version)  # Update pre-release status

    notice = build_version_notice(kozmoai_version, package_name)
...
def run_kozmoai(host, port, log_level, options, app) -> None:
    """Run Kozmoai server on localhost."""
    if platform.system() == "Windows":
        import uvicorn
        ...
        from kozmoai.server import KozmoaiApplication

        server = KozmoaiApplication(app, options)
...
def superuser(
    ...
    log_level: str = typer.Option("error", help="Logging level.", envvar="KOZMOAI_LOG_LEVEL"),
) -> None:
    """Create a superuser."""
...
# command to copy the kozmoai database from the cache to the current directory
# because now the database is stored per installation
@app.command()
def copy_db() -> None:
    """Copy the database files to the current directory.

    This function copies the 'kozmoai.db' and 'kozmoai-pre.db' files from the cache directory to the current
    directory.
    ...
    db_path = cache_dir / "kozmoai.db"
    pre_db_path = cache_dir / "kozmoai-pre.db"
    ...
    async def aapi_key():
        ...
        async with session_scope() as session:
            ...
            if not superuser:
                ...
                return None
            from kozmoai.services.database.models.api_key import ApiKey, ApiKeyCreate
            from kozmoai.services.database.models.api_key.crud import create_api_key, delete_api_key
