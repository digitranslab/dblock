...
from minerva.services.database.models.user.model import User, UserRead
...
def max_file_size_upload_fixture(monkeypatch):
    monkeypatch.setenv("MINERVA_MAX_FILE_SIZE_UPLOAD", "1")
    yield
    monkeypatch.undo()
...
def max_file_size_upload_10mb_fixture(monkeypatch):
    monkeypatch.setenv("MINERVA_MAX_FILE_SIZE_UPLOAD", "10")
    yield
    monkeypatch.undo()
