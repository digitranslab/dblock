...
from flowai.utils.constants import MESSAGE_SENDER_AI, MESSAGE_SENDER_USER
from loguru import logger
from platformdirs import user_cache_dir


@pytest.fixture
def flowai_cache_dir(tmp_path):
    """Create a temporary flowai cache directory."""
    cache_dir = tmp_path / "flowai"
    cache_dir.mkdir(parents=True)
    return cache_dir


@pytest.fixture
def sample_image(flowai_cache_dir):
    """Create a sample image file for testing."""
    # Create the test_flow directory in the cache
    flow_dir = flowai_cache_dir / "test_flow"
    ...
    real_cache_dir = Path(user_cache_dir("flowai"))
...
def test_message_prompt_serialization():
    template = "Hello, {name}!"
    message = Message.from_template(template, name="Flowai")
    assert message.text == "Hello, Flowai!"

    prompt = message.load_lc_prompt()
    assert isinstance(prompt, ChatPromptTemplate)
    assert prompt.messages[0].content == "Hello, Flowai!"
...
def test_message_with_multiple_images(sample_image, flowai_cache_dir):
    ...
    real_cache_dir = Path(user_cache_dir("flowai")) / "test_flow"
    real_cache_dir.mkdir(parents=True, exist_ok=True)
