...
from minerva.api.v1.schemas import InputValueRequest
from minerva.custom import Component
from minerva.custom.eval import eval_custom_component_code
from minerva.field_typing import Embeddings
from minerva.graph import Graph
from minerva.processing.process import run_graph_internal
...
def download_flow_from_github(name: str, version: str) -> JSONFlow:
    response = requests.get(
        f"https://raw.githubusercontent.com/digitranslab/minerva/v{version}/src/backend/base/minerva/initial_setup/starter_projects/{name}.json",
        timeout=10,
    )
...
def download_component_from_github(module: str, file_name: str, version: str) -> Component:
    ...
    response = requests.get(
        f"https://raw.githubusercontent.com/digitranslab/minerva/{version_string}/src/backend/base/minerva/components/{module}/{file_name}.py",
        timeout=10,
    )
    response.raise_for_status()
