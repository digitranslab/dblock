...
def setup_database_url(tmp_path, monkeypatch):
    ...
    original_value = os.getenv("FLOWAI_DATABASE_URL")
    monkeypatch.delenv("FLOWAI_DATABASE_URL", raising=False)
    test_db_url = f"sqlite:///{db_path}"
    monkeypatch.setenv("FLOWAI_DATABASE_URL", test_db_url)
    settings_service.set("database_url", test_db_url)
    yield
    # Restore original value if it existed
    if original_value is not None:
        monkeypatch.setenv("FLOWAI_DATABASE_URL", original_value)
...
async def test_initialize_super_user():
    ...
    from flowai.initial_setup.setup import initialize_super_user_if_needed
    from flowai.services.utils import initialize_services
...
async def test_create_starter_projects():
    ...
    from flowai.initial_setup.setup import create_or_update_starter_projects
    from flowai.interface.components import get_and_cache_all_types_dict
    from flowai.services.utils import initialize_services
...
async def test_load_flows():
    ...
    from flowai.initial_setup.setup import load_flows_from_directory
